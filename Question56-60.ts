...
let mixString : any[] = [1, "Aynn", 5, "Raza", 9, "Jafferi"]


let findStr = mixString.filter((val)=>
    (typeof val === "string") 
);
console.log(findStr)
...
let grades : number[] = [50, 44, 32, 20, 40];
...
let sum = grades.reduce((pre , curr) => 
    (pre + curr)
...
number {
let find = scores.reduce((pre: number , curr: number) => 
    (pre + curr) )
...
function makeAdder(val: number): (number :number)  => number {
